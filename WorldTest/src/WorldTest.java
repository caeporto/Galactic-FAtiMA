...
/**
 * Created by caetanoportodasilva on 23/01/17.
 */
public class WorldTest {
    ...
    private TextArea textAreaEnv;
    private String accumulator;

    public static WorldTest createWorld(String args[], TextArea textField) throws Exception{
        int i;
        ...
        if (args.length == 2){
            ...
            System.out.println("##############################");
            WorldTest wt = new WorldTest(new Integer(args[0]).intValue(),args[1],args[2], args[3], args[4],args[5],simplifiedVersion,objects,textField);
            //wt.run();
            return wt;
        }
        if (args.length < 2) {
    ...
    private WorldTest(int port, String scenery, String actionsFile, String agentLanguageFile, String userLanguageFile, String userOptionsFile, boolean simplifiedVersion, ArrayList<String> objects, TextArea textField) throws IOException {
        _scenery = scenery;
        ...
        try{
            ...
            _textInfo = "";
            this.WriteLine("Finished ALE initialization!");
    ...
    {
        accumulator += text + "\n";
        textAreaEnv.setText(accumulator);
    }

    public String getText(){
        return textAreaEnv.getText();
    }
    ...
    private LanguageEngine initializeLanguageEngine(String name, String role, String sex, String languageFile) throws SactException, LactException{
        ...
        if(role.equals("User")){
            this.WriteLine("Initializing User Language Engine(ALE)... ");
        }else{
            this.WriteLine("Initializing Agent Language Engine(ALE)... ");
        }
        this.WriteLine("Language File: " + languageFile);
        this.WriteLine("Sex: " + sex);
        this.WriteLine("Role: " + role);
    ...
    {
        this.WriteLine("Loaded actions from: " + xmlFile);
        _textInfo = this.getText();
    ...
    public void run() {
        ...
        while(true) {
            try {
                ...
                //hack for Mac
                Thread.sleep(150);
                this.WriteLine(ra.Name() + " enters the " + _scenery);
    ...
    public String Say(String speech)
    {
        ...
        if(this._agentLanguage!= null)
        {
            ...
            {
                if(e instanceof Language.SactException)
                {
                    System.out.println("LanguageEngine: "+e.getMessage());
                }
            }
        }
        return null;
    ...
    {
        this.WriteLine(user + " says to " + target + "; " + text);
    ...
    public synchronized void removeAgent(RemoteAgent ra){
        _agents.remove(ra);
        this.WriteLine(ra.Name() + " disconnected\n");
        if(_agents.isEmpty()){
            textAreaEnv.setText(_textInfo);
        }
    }

    public synchronized void removeGreta(){
        this.WriteLine(_ga + " disconnected\n");
        _ga = null;
    }
