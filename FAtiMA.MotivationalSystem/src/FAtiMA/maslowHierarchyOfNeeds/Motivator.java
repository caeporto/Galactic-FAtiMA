...
public class Motivator implements Serializable {
	...
	public static final float prepotencyDecayFactor = 2.5f; //2.5x stronger decay
	 ...
	 */
	public Motivator(String hierarchy, String motivation, short type, float decayFactor, float weight, float intensity) {
	...
	public Motivator(Motivator mot) {
		_hierarchy = mot._hierarchy;
		_motivation = mot._motivation;
		_type = mot._type;
	...
	/**
	 * Decays the motivator that is currently on the level of hierarchy with most urge
	 * @return the intensity of the motivator after being decayed
	 */
	public void DecayMotivatorWithPrepotency() {
		long deltaT;
	
		deltaT = (AgentSimulationTime.GetInstance().Time() - _t0)/1000;
		_intensity = Math.max(0, _intensityATt0 * ((float) Math.exp(-MotivatorDecayFactor * prepotencyDecayFactor * _decayFactor * deltaT)));
	}
