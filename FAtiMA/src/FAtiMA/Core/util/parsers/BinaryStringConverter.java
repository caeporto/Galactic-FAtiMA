...
public class BinaryStringConverter 
{
	...
	// the above article suggests using this lookup table is faster than
	// Integer.toHexString
	private final static String[] _hexLookupTable = 
	{ 
	...
	public static byte[] decodeStringToBinary(String input)
	{
		...
		{
			 buffer[i / 2] = (byte) ((Character.digit(input.charAt(i), 16) << 4)
	...
	{
		// a test byte array
		byte [] testdata = {0, 50, -50, 127, -128, 34, -1};
		String encoded = encodeBinaryToString(testdata);
		byte[] decoded = decodeStringToBinary(encoded);
				
		System.out.print("testdata:");
			for (int i=0; i<testdata.length; i++)
				System.out.print(" " + (0xff & testdata[i]));
		System.out.println();	
		...
		for (int i=0; i<decoded.length; i++)
			System.out.print(" " + (0xff & decoded[i]));
		System.out.println();		
