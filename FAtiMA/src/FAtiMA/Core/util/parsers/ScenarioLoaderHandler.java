...
public class ScenarioLoaderHandler extends ReflectXMLHandler{
	...
	private boolean alreadyFoundScenario;
	...
	public ScenarioLoaderHandler(String scenarioName){
		...
		this.alreadyFoundScenario = false;
	}	
	...
	public void Scenario(Attributes attributes){
		//for some reason scenarioName can't be compared to other strings
		//scenario -1 character
		//agent common substring 
		//usado para a inicializacao do agente sem essa string nao funciona (?)
		//o world pode ser inicializado com a variavel _scenarioName mas o agente nao
		String newScenario = this._scenarioName.substring(0, this._scenarioName.length()-1);
		if(newScenario.equals(attributes.getValue("name")) || this._scenarioName.equals(attributes.getValue("name"))){
